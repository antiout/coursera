...
class FunSetSuite extends FunSuite {
  ...
  test("intersect contains 2") {
    new TestSets {
      val s = union(union(union(s1, s2), s3), singletonSet(5))
      val t = union(union(singletonSet(4), s2), singletonSet(7))
      val i = intersect(s,t)
  ...
  test("Map 1,2,3 - > 4,8,12") {
    new TestSets {
      val s = union(union(s1, s2), s3)
      val r = map(s, x=>x*4)
      assert(contains(r, 4), "map 4")
      assert(contains(r, 8), "map 8")
      assert(contains(r, 12), "map 12")
